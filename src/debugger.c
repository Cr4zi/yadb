...
int debugger_init(debugger_t *debugger, char *path) {
  ...
  debugger->filenames_table =
      hashtable_init(str_equals, free, free_die_path, djb2_hash);
  debugger->breakpoints_table = hashtable_init(breakpoints_equals, NULL, free,
                                               ptr_hash); // C lambda when?
...
int initialize_debugger_dwarf(debugger_t *debugger, char *path) {
  int res = dwarf_init_path(path, NULL, 0, DW_GROUPNUMBER_ANY, NULL, NULL,
                            &debugger->dw_dbg, &debugger->dw_err);
  if (res == DW_DLV_ERROR) {
    ...
    fprintf(stderr, "Error: Files without dwarf section are not supported "
...
static bool extract_filename(char *in_full_path, char **filename,
                             char **out_full_path) {
  ...
  if (res == DW_DLV_OK) {
    for (Dwarf_Signed i = 0; i < file_count; ++i) {
      ...
      if (!access(srcfiles[i], R_OK) &&
          extract_filename(srcfiles[i], &filename, &full_path)) {
        dwarf_die_path_t *die_path =
            (dwarf_die_path_t *)malloc(sizeof(dwarf_die_path_t));
        if (!die_path)
          return NULL;
        ...
        if (!hashtable_insert(debugger->filenames_table, (void *)filename,
                              (void *)die_path)) {
          free(filename);
          free(full_path);
          free(die_path);
...
void debugger_cu_walk(debugger_t *debugger) {
  ...
  Dwarf_Half version_stamp = 0, address_size = 0, length_size = 0,
  ...
  /* This function ... smh */
  while ((res = dwarf_next_cu_header_e(
              ...
              &next_cu_header_offset, &header_cu_type, &debugger->dw_err)) ==
         DW_DLV_OK) {
...
uintptr_t debugger_get_line_addr(debugger_t *debugger,
  ...
  int res = dwarf_srclines_b(die_path->die, &version_out, &table_count,
  ...
  if (table_count == 0) {
    ...
    fprintf(stderr, "This is some experimental stuff that I don't have power "
                    "to implement now");
    return addr;
  }

  res = dwarf_srclines_from_linecontext(line_context, &line_buf, &line_count,
                                        &debugger->dw_err);
  if (res == DW_DLV_ERROR) {
...
static uintptr_t get_func_addr_in_die_helper(debugger_t *debugger,
...
static uintptr_t get_func_in_die(debugger_t *debugger, Dwarf_Die die,
  ...
  while (res != DW_DLV_NO_ENTRY) {
    uintptr_t addr = get_func_addr_in_die_helper(debugger, iter_die, func_name);
...
uintptr_t get_func_addr(debugger_t *debugger, char *func_name) {
  for (int i = 0; i < TABLE_SIZE; ++i) {
    ...
    while (entry) {
      ...
      uintptr_t addr = get_func_in_die(debugger, die_path->die, func_name);
      if (addr != 0)
        return addr;
...
unsigned char set_byte_at_offset(debugger_t *debugger, uintptr_t offset,
  ...
  errno = 0;
  long word = ptrace(PTRACE_PEEKDATA, debugger->debugee,
                     (void *)(offset + base_addr), NULL);
  ...
  if (ptrace(PTRACE_POKEDATA, debugger->debugee, (void *)(offset + base_addr),
             word) == -1) {
...
unsigned char enable_breakpoint(debugger_t *debugger, uintptr_t offset) {
  breakpoint_t *breakpoint = (breakpoint_t *)hashtable_find(
      debugger->breakpoints_table, (void *)offset);
  if (breakpoint)
    ...
    for (; entry != NULL; entry = entry->next) {
      ...
      if (breakpoint->is_enabled) {
        unsigned char original_byte =
            enable_breakpoint(debugger, (uintptr_t)entry->key);
